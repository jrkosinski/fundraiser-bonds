...
describe(constants.WHITELIST_CONTRACT_ID + ": Whitelist", function () {
    let bondToken, vault, baseToken, whitelist, securityManagerAddr;                                //contracts
    ...
    beforeEach(async function () {
        ...
        //everyone gets base token and bond token 
        await mintAll(baseToken, 1_000_000_000);
        await mintAll(bondToken, 1_000_000_000); 
    ...
    it("Whitelist Constructor", function () {
        ...
        it("can't pass bogus security manager", async function () {
            ...
            await expectRevert(
                () => deploy.deployWhitelist(bondToken.address),
    ...
    async function assertCanWithdraw(user, shouldBeAllowed = true) {
        ...
        await bondToken.connect(user).approve(vault.address, 100);
        if (shouldBeAllowed) {
