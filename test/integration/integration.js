    ...
    describe("With Normal Transfer", function () {
        describe("Simple Deposit/Withdraw", function () {
            ...
            it("multiple users deposit", async function () {
                ...
                const testVault = new TestVault(vault, bondToken, baseToken);
                ...
                /*testVault.users.forEach(async (u, i) => {
                    expect(await u.bondTokenBalance()).to.equal(u.depositAmount());
                    expect(await u.baseTokenBalance()).to.equal(mintAmount - u.depositAmount());
                });

                //check vault balance 
                const sum = testVault.sumOfDeposits;
                expect(await testVault.bondTokenBalance()).to.equal(testVault.initialBondTokenBalance);
            ...
            it("multiple users deposit and withdraw + 10% gain", async function () {
                ...
                testVault.users.forEach(async (u, i) => {
                    ...
                    expect(await u.bondTokenBalance()).to.equal(0);
                });
                ...
                expect(await testVault.bondTokenBalance()).to.equal(sumOfDeposits);
            ...
            it("multiple users deposit and withdraw + 1% gain, take half profits", async function () {
                ...
                //initial balance of 0 bond token 
                expect(await bondToken.balanceOf(addr1.address)).to.equal(0);
                expect(await bondToken.balanceOf(addr2.address)).to.equal(0);
                expect(await bondToken.balanceOf(addr3.address)).to.equal(0);
                ...
                //test vault's bond token balance 
                expect(await testVault.bondTokenBalance()).to.equal(
                    testVault.initialBondTokenBalance + (sumOfDeposits / 2)
                );
        ...
        describe("Rollover", function () {
            ...
            it("manual rollover compounding", async function () {
                ...
                expect(await testVault.user.bondTokenBalance()).to.equal(0);
            ...
            it("no rollover (withdraw all profit every round)", async function () {
                ...
                //round 1;  withdraw 1% 
                await testVault.jumpToNextWithdrawPhase(exchangeRates.ONE_PERCENT);
                await testVault.user.withdraw(testVault.convertToBondToken(await testVault.user.unrealizedProfit()));
            ...
            it("manual rollover vs. no rollover", async function () {
                ...
                await testVault.user1.withdraw(testVault.convertToBondToken(10000));
                ...
                expect(await testVault.user0.bondTokenBalance()).to.equal(0);

                expect(await user1_profit).approximately(xPercentOfy(3, principal), 1);
                expect(await testVault.user1.bondTokenBalance()).to.equal(0);
            ...
            it("auto-rollover with partial profits", async function () {
                ...
                await testVault.user0.withdraw(testVault.convertToBondToken(10000));
                await testVault.user1.withdraw(testVault.convertToBondToken(10000));
        ...
        describe("Implications of Transferring Tokens", function () {
            //NOTE: not relevant while BondToken can't be transferred
            it.skip("user can use gifted VT to exchange for just as many ST as original investor could", async function () {
                ...
                //pass all to user1
                await testVault.user.transferBondTokenTo(
                    testVault.user1.address, 'all'
                );

                //user1 passes 1/2 to user2 
                await testVault.user1.transferBondTokenTo(
            ...
            it("cannot use tokens for a different vault", async function () {
                ...
                await bondToken.connect(testVault.user.user).approve(vault2.address, 100);
