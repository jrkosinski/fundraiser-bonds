...
describe.only("Integration Fuzzing", function () {
            ...
            for (let n = 0; n < NUM_TEST_EPOCHS; n++) {
                ...
                for (let n = 0; n < NUM_TEST_ROUNDS_PER_EPOCH; n++) {
                    ...
                    for (let u of testVault.users) {
                        ...
                        //decide first to do something or not (50% chance)
                        if (mathUtils.randomBool()) {
                            const bondTokenBalance = await u.bondTokenBalance();

                            //50% chance of depositing if baseTokenAmount > 0
                            if (bondTokenBalance > 0) {
                                if (mathUtils.randomBool()) {
                                    const withdrawAmount = mathUtils.randomInt(1, bondTokenBalance);
                ...
                for (let u of testVault.users) {
                    ...
                    //expect(bondTokenBalance).to.equal(u.depositAmount());
                    ...
                    if (bondTokenBalance > 0) {
                        console.log(`withdrawing all: ${Math.floor(testVault.convertToBaseToken(bondTokenBalance))}`);
                        await u.withdrawAll();
                    }
