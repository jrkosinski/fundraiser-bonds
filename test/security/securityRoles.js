...
describe(constants.SECURITY_CONTRACT_ID + ": Security Permissions", function () {
    ...
    beforeEach(async function () {
        [admin, pauser, minter, burner, upgrader, generalManager, 
            lifecycleManager, whitelistManager, addr1, addr2, ...addrs] = await ethers.getSigners();
        ...
        //mint bond token 
        for (let u of allRoles) {
            await bondToken.mint(u.address, minDeposit);
        }
        await bondToken.mint(addr1.address, minDeposit);
        await bondToken.mint(addr2.address, minDeposit); 
    ...
    async function assertMintPermission(account, expectAllowed = true) {
        const func = async () => { await bondToken.connect(account).mint(addr1.address, 1); }
        if (expectAllowed)
            await assertCan(func);
        else
            await assertCannot(
    ...
    async function assertBurnPermission(account, expectAllowed = true) {
        const func = async () => { await bondToken.connect(account).burn(1); }
        if (expectAllowed)
            await assertCan(func);
        else
            await assertCannot(
    ...
    async function assertSetBep20OwnerPermission(account, expectAllowed = true) {
        const func = async () => { await bondToken.connect(account).assignBep20Owner(addr1.address); }
        if (expectAllowed)
            await assertCan(func);
        else
            await assertCannot(
    ...
    async function assertUpgradeVaultTokenPermission(account, expectAllowed = true) {
        ...
        const func2 = async () => {
            await bondToken.connect(account).upgradeToAndCall(v2.address, utils.encodeFunctionSignature("aCallableMethod"));
        };
        if (expectAllowed) {
            await assertCan(func1);
            await assertCan(func2);
    ...
    async function assertPauserPermissions(account, expectAllowed = true) {
        await assertPausePermission(bondToken, account, expectAllowed);
        await assertUnpausePermission(bondToken, account, expectAllowed);
