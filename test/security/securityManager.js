...
describe(constants.SECURITY_CONTRACT_ID + ": Security Manager Basics", function () {
    let vault, bondToken, baseToken, securityManager;  //contracts
    ...
    beforeEach(async function () {
        ...
        //contract
        [bondToken, vault, baseToken, whitelist, securityManager] = await deploy.deployAll(true);
    });
    ...
    describe("Restrictions", function () {
        ...
        it("cannot set zero address to bond token security manager", async function () {
            await expectRevert(
                () => bondToken.setSecurityManager(constants.ZERO_ADDRESS),
                constants.errorMessages.ZERO_ADDRESS
            )
        });
    ...
    describe("Shared Security", function () {
        let bondToken2, vault2; 
        
        beforeEach(async function () {
            bondToken2 = deploy.deployVault
        });
        
        it("security manager is shared between vault and bond token", async function () {
            expect(await vault.securityManager()).to.equal(await bondToken.securityManager());
        });
        
        it("security roles are shared between vault and bond token", async function () {
            ...
            await expect(bondToken.setSecurityManager(securityManager.address)).to.not.be.reverted;
            ...
            await expectRevert(
                () => bondToken.setSecurityManager(securityManager.address),
                constants.errorMessages.CUSTOM_ACCESS_CONTROL(
                    constants.roles.ADMIN,
                    admin.address
                )
    ...
    if (!testFlags.upgradeVault) { 
        describe("Deployer can Deploy on Behalf of Admin", function () {
            it("deploy bond token", async function () {
            ...
            it("deploy bond token and vault", async function () {
                const trueAdmin = addr1;
