...
class MockVault {
    ...
    async deposit(baseTokenAmount) {
        return this.useWholeNumberDivision ?
            await this.mathContract.baseToBondTokenAtRate(baseTokenAmount, {
                bondToken: this.exchangeRate.bondToken.toString(),
                baseToken: this.exchangeRate.baseToken.toString()
            }) :
            this.exchangeRate.baseToBondToken(baseTokenAmount);
    }

    async withdraw(bondTokenAmount) {
        return this.useWholeNumberDivision ?
            await this.mathContract.bondToBaseTokenAtRate(bondTokenAmount, {
                bondToken: this.exchangeRate.bondToken.toString(),
                baseToken: this.exchangeRate.baseToken.toString()
            }) :
            this.exchangeRate.bondToBaseToken(bondTokenAmount);
    }
}
...
describe("Tracking Exchange Rate Drift", function () {
    ...
    async function execRound(principal, expectedGainPct) {
        const bondTokenAmount = await vault.deposit(principal);
        ...
        const amountReceived = await vault.withdraw(bondTokenAmount);
