...
class TestVault {
    constructor(vaultContract, bondToken, baseToken) {
        this.vault = vaultContract;
        this.users = [];
        this.bondToken = bondToken;
        this.baseToken = baseToken;
        this.initialBaseTokenBalance = 0;
        this.initialBondTokenBalance = 0;
        this._deposits = [];
        this._exchangeRate = { bondToken: 1, baseToken: 1 };
    ...
    async init() {
        ...
        this.initialBondTokenBalance = parseInt(await this.bondToken.balanceOf(this.vault.address));
    ...
    async bondTokenBalance() {
        return parseInt(await this.bondToken.balanceOf(this.vault.address));
    }
    ...
    convertToBaseToken(bondTokenAmount) {
        return (bondTokenAmount * this._exchangeRate.baseToken) / this._exchangeRate.bondToken;
    }

    convertToBondToken(baseTokenAmount) {
        return (baseTokenAmount * this._exchangeRate.bondToken) / this._exchangeRate.baseToken;
    }
    ...
    async nextPhase(exchangeRate) {
        ...
        await this.vault.progressToNextPhase({
            bondToken: exchangeRate.bondToken.toString(),
...
class TestVaultUser {
    constructor(vault, user) {
        ...
        this._initialBondTokenBalance = 0;
    ...
    get initialBondTokenBalance() { return this._initialBondTokenBalance; }
    get expectedResults() { return this._ledger; }

    async init() {
        this._initialBaseTokenBalance = parseInt(await this.baseTokenBalance());
        this._initialBondTokenBalance = parseInt(await this.bondTokenBalance());
    }

    async netWorthAt(exchangeRate) {
        const bondTokenBal = await this.bondTokenBalance();
        const baseTokenBal = await this.baseTokenBalance();

        return baseTokenBal + ExchangeRate.copy(exchangeRate).bondToBaseToken(bondTokenBal);
    }

    async bondTokenBalance() {
        return parseInt(await this.vault.bondToken.balanceOf(this.user.address));
    }
    ...
    async withdraw(amount) {
        amount = await convertAmount(amount, () => this.bondTokenBalance());
    ...
    async withdrawPercent(percent) {
        const myBalance = await this.bondTokenBalance();
    ...
    async unrealizedProfit() {
        return (this.vault.convertToBaseToken(await this.bondTokenBalance()) - this._initialBaseTokenBalance)
    }
    ...
    async transferBondTokenTo(recipAddress, amount) {
        amount = await convertAmount(amount, () => this.bondTokenBalance());
        await this.vault.bondToken.connect(this.user).transfer(recipAddress, amount);
    }
