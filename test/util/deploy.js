...
async function deployVault(
    ...
    let bondToken;
    if (bondTokenAddress)
        bondToken = await ethers.getContractAt(constants.TOKEN_CONTRACT_ID, bondTokenAddress);
    else
        bondToken = await deployToken(securityManagerAddress); 
    ...
    await setInitialBondTokenRoles(vault, bondToken);

    //associate vault with token 
    if (associateBondToken) 
...
async function deployAll(withWhitelist = true) {
    ...
    const bondToken = await getBondToken(vault);
    ...
    return [
        bondToken, vault, baseToken, whitelist, securityManager
    ];
}

async function deployTestToken() {
    return await utils.deployContractSilent("TestBondToken");
}

async function deployVaultWithTestToken() {
    const baseToken = await deployStableCoin();
    const bondToken = await deployTestToken();
...
async function upgradeToken(address) {
    return await utils.upgradeProxy("BondTokenV2", address);
}
...
async function getBondToken(vaultContract) {
    return await ethers.getContractAt(constants.TOKEN_CONTRACT_ID, await vaultContract.bondToken()); 
}
