...
describe(constants.TOKEN_CONTRACT_ID + ": ERC20 Transfer", function () {
    ...
    describe("Approve and Transfer", function () {
        describe("Approve Single", function () {
            ...
            it("anyone can approve and transfer to non-vault address", async function () {
                const amount = 100;
            ...
            it("anyone can approve and transfer to a third non-vault address", async function () {
                const amount = 100;
                await token.connect(addr1).approve(addr2.address, amount);
                expect(await token.allowance(addr1.address, addr2.address)).to.equal(amount);
            ...
            it("anyone cannot approve and double transfer", async function () {
                const amount = 100;
                await token.connect(addr1).approve(addr2.address, amount);
            ...
            it("transfer to non-vault address decreases allowance", async function () {
    ...
    describe("Events", function () {
        ...
        it("transfer event fires on transfer to non-vault address", async () => {
        ...
        it("transfer event fires on transferFrom to non-vault address", async () => {
