...
describe(constants.VAULT_CONTRACT_ID + ": Automatic Minting", function () {
    ...
    beforeEach(async function () {
        ...
        await bondToken.mint(vault.address, initialBalance); 
        initialTotalSupply = parseInt(await bondToken.totalSupply());
    ...
    describe("Initial State", function () {
        it("initial vault balance", async function () {
            expect(await bondToken.balanceOf(vault.address)).to.equal(initialBalance); 
        });
    ...
    describe("Use of Automatic Minting", function () {
        it("use less than supply in vault", async function () {
            ...
            expect(await bondToken.totalSupply()).to.equal(initialTotalSupply);
            
            //balance in vault has decreased 
            expect(await bondToken.balanceOf(vault.address)).to.equal(initialBalance - depositAmount);
        });
        ...
        it("use more than supply in vault", async function () {
            ...
            //total supply has stayed the same (no minting)
            expect(await bondToken.totalSupply()).to.equal(initialTotalSupply + (initialBalance*2));

            //balance in vault has decreased 
            expect(await bondToken.balanceOf(vault.address)).to.equal(0);
        });
        ...
        it("use one less than supply in vault", async function () {
            ...
            //balance in vault has decreased 
            expect(await bondToken.balanceOf(vault.address)).to.equal(1);
        });
        ...
        it("use one more than supply in vault", async function () {
            ...
            //total supply has stayed the same (no minting)
            expect(await bondToken.totalSupply()).to.equal(initialTotalSupply + 1);
