...
describe(constants.VAULT_CONTRACT_ID + ": Direct Transfer Withdraw", function () {
    ...
    beforeEach(async function () {
        ...
        bondToken.mint(vault.address, 1_000_000_000);
    ...
    async function withdrawFromVault(depositor, amount) {
        await bondToken.connect(depositor).transfer(vault.address, amount);
    }
    ...
    describe("Withdraw Direct", function () {
        it("successful withdraw", async function () {
            ...
            expect(depositor_bondToken_2).to.equal(depositor_bondToken_1 - withdrawAmount);
            expect(vault_baseToken_2).to.equal(vault_baseToken_1 - withdrawAmount);
            expect(vault_bondToken_2).to.equal(vault_bondToken_1 + withdrawAmount);
        });
        ...
        it("withdraw by delegate", async function () {
            ...
            const vault_bondToken_1 = parseInt(await bondToken.balanceOf(vault.address));
            const depositor_bondToken_1 = parseInt(await bondToken.balanceOf(depositor.address));
            const spender_bondToken_1 = parseInt(await bondToken.balanceOf(spender.address));
            ...
            await bondToken.connect(depositor).approve(spender.address, withdrawAmount); 

            //withdraw
            await bondToken.connect(spender).transferFrom(depositor.address, vault.address, withdrawAmount); 
            ...
            const vault_bondToken_2 = parseInt(await bondToken.balanceOf(vault.address));
            const depositor_bondToken_2 = parseInt(await bondToken.balanceOf(depositor.address));
            const spender_bondToken_2 = parseInt(await bondToken.balanceOf(spender.address));
            ...
            expect(spender_bondToken_1).to.equal(spender_bondToken_2);
        });
        ...
        it("can't withdraw if vault doesn't have enough base token", async function () {
            ...
            await bondToken.mint(depositor.address, withdrawAmount);

            //deposit first 
            await depositToVault(depositAmount);

            //mint some extra bondToken to user
            await bondToken.mint(depositor.address, 100);

            await vault.progressToNextPhase(constants.exchangeRates.PARITY); //locked
            await vault.progressToNextPhase({ bondToken: 1, baseToken: 4 }); //withdraw
        ...
        it("can withdraw zero amount", async function () {
            ...
            await expect(bondToken.connect(depositor).transfer(vault.address, withdrawAmount)).to.not.be.reverted;
        });
        ...
        it("can't withdraw more than balance", async function () {
            ...
            const withdrawAmount = (await bondToken.balanceOf(depositor.address)) + 1;
        ...
        it("withdraw with base token transfer failure", async function () {
            ...
            const withdrawAmount = (await bondToken.balanceOf(depositor.address));

            await vault.progressToNextPhase(constants.exchangeRates.PARITY); //locked
            await vault.progressToNextPhase({ bondToken: 1, baseToken: 1 }); //withdraw
    ...
    describe("Security", function () {
        it("only bondToken can call withdrawDirect directly", async () => {
            await expectRevert(
        ...
        it("only vault can call withdrawDirect directly", async () => {
            await expectRevert(
                () => bondToken.transferFromInternal(owner.address, owner.address, 1),
            ...
            await grantRole(vault, constants.roles.ADMIN, testAddr.address);
            await expectRevert(
                () => bondToken.transferFromInternal(testAddr.address, owner.address, 1),
                constants.errorMessages.VAULT_ONLY
            );
    ...
    describe("Constraints", function () {
        it("can't transfer directly when paused", async () => {
            ...
            await expectRevert(
                () => bondToken.connect(depositor).transfer(vault.address, depositAmount),
        ...
        it("can't transferFrom directly when not in phase", async () => {
            ...
            //deposit 
            await bondToken.mint(addr2.address, depositAmount);

            await vault.progressToNextPhase([1, 1]);
            await bondToken.connect(addr2).approve(depositor.address,depositAmount); 
            await expectRevert(
                () => bondToken.connect(depositor).transferFrom(addr2.address, vault.address, depositAmount),
                constants.errorMessages.VAULT_OUT_OF_PHASE
            );
        });
    });
    ...
    describe.skip("Events", function () {
        ...
        it.skip("Withdraw event fires on withdraw", async () => {
            ...
            await expectEvent(() => 
                bondToken.connect(depositor).transfer(vault.address, amountIn),
