...
{            
     ...
     *      - if caller's balance of BondToken is < `amount`
    ...
    /**
     * This is just between the Vault and the Bond Token. It's private. Like just between them two. 
     * 
        ...
        returns (bool)
    {
        return 
            ...
            interfaceId == type(IBondToken).interfaceId; 
    }
    
    /**
     * Allows authorized user to associate this BondToken with its vault. 
     * 
     * This is optional, but is used for direct transfer of BondToken directly to the Vault, bypassing the 
    ...
    function setVaultAddress(address addr) external onlyRole(ADMIN_ROLE) {
        ...
        if (address(vault.bondToken()) != address(this))
            revert InvalidVaultAddress();
